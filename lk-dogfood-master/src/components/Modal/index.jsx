...
const Modal = ({active, setActive, setUser}) => {
    ...
    const sendForm = async (e) => {
        ...
        let reg = "https://api.react-learning.ru/signup"//выход
        ...
        if (!data.err) {
            if (!auth) {
                ...
                if (!dataLog.err) {
                    localStorage.setItem("bandUser", dataLog.data.name);
                    localStorage.setItem("bandToken", dataLog.token);
                    localStorage.setItem('bandId', dataLog.data._id);
                    clearForm();
                    setActive(false);
                ...
                if (!data.err) {
                    localStorage.setItem("bandUser", data.data.name);
                    localStorage.setItem("bandToken", data.token);
                    localStorage.setItem('bandId', data.data._id);
                    clearForm();
                    setActive(false);
        ...
        >
            <div className="modal">
                ...
                <form onSubmit={sendForm}>
                    ...
                    {!auth && <label>
                        Повторить пароль
                        <input type="password" value={testPwd}
